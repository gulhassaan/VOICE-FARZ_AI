...
import { FiMenu, FiEdit, FiTrash2, FiLogOut, FiSearch, FiMessageSquare } from "react-icons/fi";
...
const Sidebar = () => {
  ...
  const fetchHistoryTitles = async () => {
    ...
    try {
      const response = await axios.get("https://speechinsightsweb.azurewebsites.net/speech_history/", {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://speechinsightsweb.azurewebsites.net/speech_history/${id}/delete/`, {
        headers: {
          Authorization: "Bearer " + token,
          "Content-Type": "application/json",
        },
      ...
      if (response.data && response.data.message === "Speech thread deleted successfully") {
  ...
  const clearAllHistory = async () => {
    ...
    try {
      ...
      Swal.fire("Error", "Failed to clear history. Please try again later.", "error");
    }
  };
  ...
  const { today, yesterday, previousWeek, pastHistory } = categorizeHistory(filteredTitles);
    ...
    <>
      ...
      >
        <div className="flex-1 overflow-y-auto scrollbar-sidebar">
          <div className="flex space-x-4 items-center justify-center px-6 py-4">
            <img
              src={Logo}
              alt="Farz AI Logo"
          ...
          <div className="px-4 flex items-center justify-between">
            <NavLink       onClick={toggleSidebar} to="/dashboard" className="bg-[#f2911b] py-2 px-6 lg:px-12 rounded-full text-white font-regular flex items-center">
            ...
            {loading ? (
              ...
              filteredTitles.length === 0 ? (
                ...
                <>
                  {today.map((title, index) => (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs truncate">{truncateTitle(title.title, 30)}</span>
                      </div>
                      {location.pathname === `/conversation/${title.id}` && (
                  ...
                  {yesterday.length > 0 && <div className="text-gray-500 text-xs mt-4">Yesterday</div>}
                  ...
                  {previousWeek.length > 0 && <div className="text-gray-500 text-xs mt-4">Previous 7 Days</div>}
                  ...
                  {pastHistory.length > 0 && <div className="text-gray-500 text-xs mt-4">Past History</div>}
