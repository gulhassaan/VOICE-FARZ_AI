...
import {
  faAngleDown, faAngleUp, faCopy, faWandMagicSparkles, faPause,
  faRedo, faTimes, faEdit, faDownload, faShareAlt, faSave,
  faMicrophone, faStop
...
const RecordScreen = () => {
  ...
  const handleStart = async () => {
    if (!isRecording) {
        try {
            // Fetch the token and region from your API
            const tokenResponse = await axios.get('https://voiceamplifiedbackendserver.eastus.cloudapp.azure.com/azure_speech_token/', {
                headers: {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then((stream) => {
                    ...
                    // Initialize Speech Recognizer with the token and region from the API
                    ...
                    recognizer.recognizing = (s, e) => {
                        ...
                        setRealTimeTranscript(e.result.text);
                    };
                ...
                .catch((error) => {
                    ...
                    alert("Error accessing microphone. Please ensure the microphone is available and permission is granted.");
                });
        } catch (error) {
            console.error('Error fetching token or initializing speech recognizer:', error);
        }
    }
};
...
const setCaretPosition = (el, offset) => {
  ...
  const handleSave = () => {
    ...
    // Append the final real-time transcript to the main transcript and clear the real-time transcript
    setTranscript(prev => prev + ' ' + realTimeTranscript);
...
useEffect(() => {
    // Save the updated transcript automatically
    const saveTranscript = async () => {
        try {
            await axios.post('https://your-api-endpoint.com/save-transcript', { transcript });
